...
use crate::config::{config_hash, ConfigError, DistanceMode, SearchConfig};
...
pub struct EnumeratorSnapshot {
    ...
    pub(crate) heap: Vec<QueueItem>,
    /// best[depth] is a list of (chars, cost) pairs.
    pub(crate) best: Vec<Vec<(Vec<char>, u32)>>,
    pub(crate) finalized_global: Vec<Vec<char>>,
    pub(crate) stats: EnumeratorStats,
}
...
mod tests {
    use super::{
        Candidate, EnumeratorSnapshot, PipelinedOrderedCandidateEnumerator, QueueItem,
        SnapshotError,
    };
    use crate::config::{config_hash, ConfigError, DistanceMode, EditOps, SearchConfig};
    use std::collections::{BinaryHeap, HashMap, HashSet};
